    ...
    if(!head){
        ...
        while( (p_fast!=NULL) && (p_fast->next != NULL)
         && (p_fast->next->next != NULL)
          ...
          && (p_fast->next->next->next->next != NULL)
         ){
...
void reverse_linked_list(void){
    ...
    if((head ==NULL) || (head->next==NULL)){
        printf("Empty Linked List or one member");
    }else{
        tempNode= head, prevNode= head;
        nextNode= head->next;
        while(nextNode!=NULL){
...
void three_fourth_node(void){
    ...
    if(!head){
        ...
        printf("three fourth node is %d\n", p_slow->data);
    } 
}
void print_before_last_node (void){
    node *p=head;
    if((head !=NULL) && (head->next!=NULL)){
        while(p->next->next !=NULL){
            p=p->next;
        }
        printf("Before last node is %d\n", p->data);
    }else{
        printf("Linked list in empty or only 1 member \n");
    }
}
int main() {
    // Write C code here
    insert_position(20, 30);
    print_before_last_node();
    insert_end(4);
    print_node();
    print_before_last_node();
    print_node();
...
Linked list in empty or only 1 member 
4 
Linked list in empty or only 1 member 
4 
4 20 
Before last node is 4
...
Middle node is 463
One fourth node is 166
three fourth node is 694
Print Reverse
